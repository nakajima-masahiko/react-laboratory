...
| 名前        | 型                 | 必須 | 既定値 | 説明                               |
| ----------- | ------------------ | ---- | ------ | ---------------------------------- |
| `data`      | `CandleData[]`     | Yes  | -      | 時系列 OHLC。**`time` 昇順で渡すこと**。空配列可 |
| `chartType` | `ChartType`        | Yes  | -      | `"candlestick"` / `"line"`         |
| `theme`     | `ChartTheme`       | Yes  | -      | 描画色一式                         |
| `timeframeMs` | `number`         | No   | `86_400_000` | 足の時間幅 (ms)。将来の時間足制御拡張用 |
...
- `d3.scaleTime()`
- domain: `[times[0], times[times.length - 1]]` を基本とする
- API 契約として `data` は `time` 昇順を要求する
- 防御として昇順でないデータを検出した場合は `min(time)` / `max(time)` で domain を補正し、開発時に warning を出す
- range: `[plot.left, plot.right]`
- データ 0 件 / 1 件も例外にならないようフォールバック domain を用意
- ticks は `xScale.ticks(6)` で生成
- 目盛りラベルは `timeframeMs` に応じて可読性優先で切り替える
  - `timeframeMs < 1日` (イントラデイ): `%H:%M:%S` を基本表示し、日付が切り替わる tick は `%Y/%m/%d %H:%M:%S`
  - `timeframeMs >= 1日`: `%Y/%m/%d` を表示し、同日の重複ラベルは空文字にして間引く
...
| `tooltipBackground`       | Tooltip パネル背景                       |
| `tooltipText`             | Tooltip 文字色                           |
| `tooltipShadow`           | Tooltip パネルのシャドウ                 |
...
これにより「論理座標 (CSS px) でコードを書き、表示は常に高 DPI」という構造を維持する。DPR 変動追従は `devicePixelRatio` を state 管理し、`matchMedia('(resolution: Xdppx)')` の change と `resize` を契機に更新して `useEffect` 再描画へ接続する。
...
- Tooltip パネルの背景・文字色・shadow は `theme.tooltipBackground` / `theme.tooltipText` / `theme.tooltipShadow` を使用する
- ガイドは `pointer-events: none` とし、操作を阻害しない
- Tooltip の横位置は `clamp(8, x + 14, width - tooltipWidth - 8)` で制御し、狭幅でも画面外にはみ出さない
- マウス移動時の Tooltip 更新は `requestAnimationFrame` で間引き、再レンダー頻度を抑制する

## アクセシビリティ

- Canvas 要素には `role="img"` と `aria-label` を付与する
- `aria-label` には期間・最新終値 (またはデータなし) の要約を含める
