    ...
    <div className="ccl-console-grid">
      <section className="ccl-panel" aria-labelledby="ccl-dataset-heading">
        ...
        <label className="ccl-field">
          <span>Size</span>
          <select
            value={props.datasetSize}
            onChange={event => props.setDatasetSize(Number(event.target.value) as DatasetSize)}
          >
        ...
        <button type="button" onClick={() => void props.loadDataset(props.datasetSize)} disabled={props.isBusy}>
          Load dataset
        </button>
        <button type="button" onClick={props.reseed} disabled={props.isBusy}>
          Reseed + load
        </button>
      </section>
      ...
      <section className="ccl-panel" aria-labelledby="ccl-realtime-heading">
        ...
        <label className="ccl-field">
          <span>Rate</span>
          <select
            value={props.realtimeMode}
            onChange={event => props.setRealtimeMode(event.target.value as RealtimeMode)}
          >
            <option value="off">Off</option>
            <option value="1">1 tick/sec</option>
            <option value="10">10 ticks/sec</option>
            <option value="60">60 ticks/sec</option>
            <option value="120">120 ticks/sec</option>
            <option value="200">200 ticks/sec</option>
          </select>
        </label>
        <button type="button" onClick={() => void props.runBurst()} disabled={props.isBusy}>
          Burst 1,000 ticks
        </button>
      </section>
      ...
      <section className="ccl-panel ccl-operations" aria-labelledby="ccl-operations-heading">
        ...
        <button type="button" onClick={() => props.chartRef.current?.zoomIn()}>
          Zoom in
        </button>
        <button type="button" onClick={() => props.chartRef.current?.zoomOut()}>
          Zoom out
        </button>
        <button type="button" onClick={props.appendCandle}>
          Append one candle
        </button>
        <button type="button" onClick={() => props.sendTick(false)}>
          Same-bucket tick
        </button>
        <button type="button" onClick={() => props.sendTick(true)}>
          New-bucket tick
        </button>
        <span className="ccl-control-group" aria-label="Visible range presets">
          <button type="button" onClick={() => props.showRangePreset('first')}>
            First 10%
          </button>
          <button type="button" onClick={() => props.showRangePreset('middle')}>
            Middle 10%
          </button>
        ...
        <div className="ccl-button-row">
          {(['pan', 'inspect'] as const).map(mode => (
            <button
              key={mode}
              type="button"
              aria-pressed={props.interactionMode === mode}
              data-active={props.interactionMode === mode}
              onClick={() => props.setInteractionMode(mode)}
            >
              {mode === 'pan' ? 'Pan' : 'Inspect'}
            </button>
          ))}
        </div>
        <p className="ccl-guidance">
          In Inspect, press in the plot to pin the nearest candle, drag candle-by-candle, and release to keep it
          pinned. Press outside the plot to clear it.
        </p>
      </section>
      ...
      <section className="ccl-panel" aria-labelledby="ccl-navigator-heading">
        ...
        <button
          type="button"
          aria-pressed={props.navigator.visible}
          data-active={props.navigator.visible}
        ...
        <label className="ccl-field">
          ...
          <input
            ...
            max="160"
            step="4"
            value={props.navigator.height}
            onChange={event => props.setNavigatorHeight(event.currentTarget.valueAsNumber)}
          />
        </label>
        <label className="ccl-field">
          ...
          <input
            ...
            max="5000"
            step="100"
            value={props.navigator.maxOverviewPoints}
            onChange={event => props.setNavigatorMaxOverviewPoints(event.currentTarget.valueAsNumber)}
          />
        </label>
      </section>
      ...
      <section className="ccl-panel ccl-indicators" aria-labelledby="ccl-indicators-heading">
        ...
        <div className="ccl-button-row" aria-label="Indicator presets">
          <button type="button" onClick={() => props.setPreset('base')}>
            Base only
          </button>
          <button type="button" onClick={() => props.setPreset('trend')}>
            Trend
          </button>
          <button type="button" onClick={() => props.setPreset('momentum')}>
            Momentum
          </button>
          <button type="button" onClick={() => props.setPreset('all')}>
            All visible
          </button>
        </div>
        <div className="ccl-button-row" aria-label="Individual indicators">
          {INDICATORS.map(indicator => (
            <button
              key={indicator.key}
              type="button"
              data-active={props.visible[indicator.key]}
              aria-pressed={props.visible[indicator.key]}
              onClick={() => props.toggleIndicator(indicator.key)}
            >
              {indicator.label}
            </button>
          ))}
